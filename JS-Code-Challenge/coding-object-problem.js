...
// 6. map, filter, sort, reduce, and group by in JavaScript
...
// const res = user.map((item, index) => item.name)

// const res = user.filter((item, index) => item.age > 20).map((item, index) => item.name);

const res = user.map((item, index) => ({
    ...item,
    age: item.age + 5,
})).sort((a, b) => a.age - b.age).reduce((oldest, user) =>
    user.age > oldest.age ? user.name : oldest
);


// const res = Object.groupBy(user, ({ city }) => city);
